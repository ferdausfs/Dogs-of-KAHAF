    ...
    private fun parseOutput(scores: FloatArray, size: Int, threshold: Float): AiResult {
        ...
        return when (size) {
            ...
            5 -> {
                ...
                // NSFW score = hentai + porn + weighted sexy
                ...
                val isUnsafe = unsafeScore >= threshold || sexy >= 0.45f
                ...
                AiResult(
                    isUnsafe    = isUnsafe,
                    unsafeScore = if (isUnsafe) maxOf(unsafeScore, if (sexy >= 0.45f) sexy else 0f) else unsafeScore,
                    label       = if (isUnsafe) "NSFW: $dominantLabel (${(unsafeScore * 100).toInt()}%)" else "Safe: $dominantLabel"
                )
            }
            else -> {
