...
interface AppDao {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM managed_apps WHERE packageName = :pkg AND isBlocked = 1)")
    suspend fun isAppBlocked(pkg: String): Boolean

    @Query("SELECT EXISTS(SELECT 1 FROM managed_apps WHERE packageName = :pkg AND isWhitelisted = 1)")
    ...
    @Query("DELETE FROM managed_apps WHERE packageName = :pkg")
